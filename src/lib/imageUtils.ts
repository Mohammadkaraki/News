...
const ALLOWED_DOMAINS = [
  ...
  'localhost:3000'   // Allow frontend server images
];
...
export function isValidImageUrl(url: string): boolean {
  ...
  if (url.startsWith('http://') || url.startsWith('https://')) {
    try {
      ...
      // Check if domain is in allowed list (include port for localhost)
      const hostWithPort = urlObj.port ? `${urlObj.hostname}:${urlObj.port}` : urlObj.hostname;
      return ALLOWED_DOMAINS.includes(hostWithPort) || ALLOWED_DOMAINS.includes(urlObj.hostname);
    } catch (error) {
      return false;
    }
  }
...
export function getSafeImageUrl(url: string, fallbackCategory?: string): string {
  ...
  console.log('Using fallback image for invalid URL:', url);
 ...
 * @param author - The author object (can be null/undefined)
 * @returns object with safe author properties
 */
